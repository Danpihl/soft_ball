...
int edges_array[2][NUM_EDGES] = {{0, 0, 4, 0, 9, 9, 5, 4, 5, 8, 8, 10, 8, 3, 5, 5, 2, 2, 7, 7, 7, 6, 7, 11, 11, 0, 1, 9, 9, 11},
                          {4, 1, 1, 9, 4, 5, 4, 8, 8, 1, 10, 1, 3, 10, 3, 2, 3, 7, 3, 10, 6, 10, 11, 6, 0, 6, 6, 11, 2, 2}};

float points_array[3][NUM_POINTS] = {{-0.5257311121, 0.5257311121, -0.5257311121,  0.5257311121,            0,             0,             0,             0, 0.8506508084, -0.8506508084,  0.8506508084, -0.8506508084},
                       {0,            0,             0,             0, 0.8506508084,  0.8506508084, -0.8506508084, -0.8506508084, 0.5257311121,  0.5257311121, -0.5257311121, -0.5257311121},
                       {0.8506508084, 0.8506508084, -0.8506508084, -0.8506508084, 0.5257311121, -0.5257311121,  0.5257311121, -0.5257311121,            0,             0,             0,             0}};
...
void calculateNewPointsAndEdges(Edgelist edgelist, std::vector<Point> P, Facelist facelist, Edgelist &new_edges, std::vector<Point> &Pn)
{
    Pn = P;
...
void subdivide(Edgelist edgelist, std::vector<Point> P, Facelist facelist, Edgelist &new_edges, std::vector<Point> &Pn, Facelist &new_faces)
{
    calculateNewPointsAndEdges(edgelist, P, facelist, new_edges, Pn);
    new_faces = calculateFaces(new_edges);
}

void findPointConnectivity(Edgelist edgelist, std::vector<Point> P, std::vector<std::vector<int>> &connectivity, std::vector<int> &num_edges)
{
...
void findPointsIndicesToEdgeIndices(std::vector<std::vector<int>> &point_indices_to_edge_index, Edgelist edgelist, std::vector<Point> P)
{
